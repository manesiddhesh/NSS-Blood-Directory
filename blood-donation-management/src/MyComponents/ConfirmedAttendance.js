...
export const ConfirmedAttendance = () => {
  ...
  const loadRegisteredVolunteers = async () => {
    let response = await Axios.get(`http://localhost:3001/attendance_final/${campId}`);
    let campDetails = await Axios.get(`http://localhost:3001/getCamp/${campId}`);
    let registeredApproved = await Axios.get(`http://localhost:3001/final_attendance/${campId}/approved`);
  ...
  const handleApprove = async (status, id) => {
    Axios.put(`http://localhost:3001/approveattendance_final/${id}/${status}`, {
      id,
      status
    }).then(() => {
      if (status === "approved") {
      ...
      <div className="container">
        ...
        <ToastContainer />
        {
          campDetails.map((data) => {
            return (
              <>
                <h6 align="center">Venue: {data.venue}<br /> Date: {data.date.replaceAll('-', '/')} <br /> Organizer: {data.organizer}
                  {/* Total Volunteers Need: {data.volunteers_needed}*/}<br/>Count of Volunteers that marked Attendance: {registeredVolunteers.length}</h6> 
